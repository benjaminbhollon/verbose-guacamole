...
// Restore panel state
let panelState = localStorage.panelState
  ? JSON.parse(localStorage.panelState)
  : {
      git: true,
      fileTree: true,
    };
...
function updatePageXY(event) {
  ...
  if (cursorX < 15) {
    if (!q(".tab.left").classList.contains("show"))
      q(".tab.left").classList.add("show");
...
function toggleSearch(eventType, event) {
  if (
    eventType === "blur" &&
    event.relatedTarget &&
    event.relatedTarget.classList.contains("fa-search")
  )
    return;
...
function startMoveItem(event) {
  ...
  const idToMove =
    event.currentTarget.tagName === "SUMMARY"
      ? event.currentTarget.parentNode.id
      : event.currentTarget.id;
  currentlyDragging = api
    .flatten(api.getProject().index)
    .find((i) => api.idFromPath(i.path) === idToMove).path;
}
...
function moveItem(event, index, main = false) {
  event.stopPropagation();
  const target = event.path.find((e) => e.tagName === "DETAILS")
    ? api
        .flatten(api.getProject().index)
        .find(
          (f) =>
            api.idFromPath(f.path) ===
            event.path.find((e) => e.tagName === "DETAILS").id
        ).path
    : false;
  ...
  if (
    ...
    event.toElement.id === "fileTree__actions"
  )
    order = true;

  // Check if moving folder into itself
  if (typeof currentlyDragging.children !== "undefined") {
    if (event.path.find((e) => e.id === api.idFromPath(currentlyDragging.path)))
      return;
  }
...
document.getElementById("editor").addEventListener("contextmenu", (event) => {
  if (event.path[0].classList.contains("cm-spell-error")) {
    ...
    const suggestions = api
      ...
      .map(
        (w) =>
          `<span title='Correct to "${w}"' onclick="spellCheckReplace('${w}');">${w}</span>`
      );
...
function spellCheckReplace(word) {
  ...
  const lines = [...qA(".CodeMirror-line")].map((l) =>
    l.innerText.replace("​", "")
  );
...
function createLabel() {
  return api.createLabel(
    q("#createLabel__name").value,
    q("#createLabel__color").value,
    q("#createLabel__description").value
  );
}
