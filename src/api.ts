...
export class Api {
  ...
  addToDictionary(w: any)  {
    ...
    fs.writeFileSync(path.resolve(this.appPath, './customDictionary.txt'), this.customDictionary.join('\n'));
  ...
  cancelSprint() {
    ...
    this.q('#wordSprint__status').innerText =
  ...
  async checkout (what, editable, stash = true) {
    ...
    this.project = JSON.parse(fs.readFileSync(this.projectPath, {
      encoding:'utf8',
      flag:'r'
    }));
    this.currentFile = this.flatten(this.project.index).filter(i => typeof i.children === 'undefined')[0];
  ...
  checkWord(w)  {
    if (
      this.customDictionary.indexOf(w) !== -1 ||
      !isNaN(w)
    ) return true;
  ...
  async commit(m) {
    ...
    const message = m ? m : (<HTMLTextAreaElement>document.getElementById('git__commitText')).value;
  ...
  createItem(type, first = false) {
    ...
    if (parent === null) {
      var parentFile = this.flatten(this.project.index).find(i => this.idFromPath(i.path) === folder.id);
    ...
    setTimeout(() => {
      if (type === 'file') {
        ...
        if (item) item.click();
        if (!first) this.startRename(document.getElementById(this.idFromPath(filePath) + '__filename'));
      } else {
        ...
        (<HTMLFormElement>document.getElementById(this.idFromPath(filePath))).open = true;
      }
    }, 0);
  }
  ...
  deleteItem() {
    ...
    let file = this.flatten(this.project.index).find(i => this.idFromPath(i.path) === (item.tagName === 'SPAN' ? item.id : item.parentNode.id));
    ...
    setTimeout(() => {
      const foundCurrent = this.flatten(this.project.index).find(f => this.idFromPath(f.path) === this.project.openFile);
      if (typeof foundCurrent === 'undefined') {
        if (this.flatten(this.project.index).filter(i => typeof i.children === 'undefined').length) {
          this.currentFile = this.flatten(this.project.index).filter(i => typeof i.children === 'undefined')[0];
          const item = document.getElementById(this.idFromPath(this.currentFile.path));
  ...
  fileName() {
    ...
    return (new Array(16)).map(()=>chars[Math.floor(Math.random() * chars.length)]).join('') + '.md';
  }
  flatten(arr)  {
    let newArr = arr;
    newArr = arr.map(i => {
  ...
  focusItem(id)  {
    ...
    if (item) {
      const element = item.tagName === 'SPAN' ?
        item :
      ...
      if (element) {
        ...
        if (element.classList.contains('active') && event && event.type !== 'contextmenu')
          return this.startRename(element.tagName === 'SPAN' ? element.querySelector('.filename') : element);
  ...
  async init(params) {
    ...
    try {
      this.customDictionary = fs.readFileSync(path.resolve(this.appPath, './customDictionary.txt'), {
        encoding:'utf8',
        flag:'r'
      }).split('\n').filter(l => l.length);
    } catch (err) {
      console.error(err);
      fs.writeFileSync(path.resolve(this.appPath, './customDictionary.txt'), '');
    }
    ...
    if (params.new) {
      ...
      fs.writeFile(
        path.resolve(path.dirname(this.projectPath), this.project.index[0].path),
        '',
        {
          encoding: 'utf8',
          flag: 'w'
      ...
      fs.writeFileSync(path.resolve(path.dirname(this.projectPath), '.gitignore'), '.lock');
    } else {
      if (this.gitEnabled) {
        if ((await this.git.branch()).all.length <= 0) { // Project started without git
    console.info('Creating initial commit...');
    await this.git.add('./*');
    await this.git.commit('Create project');
        }
        if ((await this.git.branch()).current !== 'master') await this.checkout('master', true);
      ...
      this.flatten(this.project.index)
        ...
        .forEach(file => {
          file.words = this.wordCount(fs.readFileSync(path.resolve(path.dirname(this.projectPath), file.path), {
            encoding:'utf8',
            flag:'r'
          }));
        });
      ...
      // with v0.1.0
      if (typeof this.project.openFolders === 'undefined') this.project.openFolders = [];

      // with <v0.1.2
      if (typeof this.project.openFile === 'undefined') this.project.openFile = this.idFromPath(this.currentFile.path);
      ...
      if (typeof this.project.history === 'undefined') this.project.history = {};
      if (typeof this.project.history.wordCount === 'undefined') this.project.history.wordCount = {};
  ...
  labelFile(fileId, labelId) {
    const file = this.flatten(this.project.index)
      .find(f => this.idFromPath(f.path) === fileId);
  ...
  moveItem(p, t, c, index, order, main = false) {
    const parent = p ? this.flatten(this.project.index).find(f => f.path === p) : {children: this.project.index};
    const target = t ? this.flatten(this.project.index).find(f => f.path === t) : {children: this.project.index};
    const currentlyDragging = this.flatten(this.project.index).find(f => f.path === c);
  ...
  openFile(p, n, first = false) {
    if (this.currentFile === this.flatten(this.project.index).find(i => i.path === p) && !first) return;
    this.resetEditor();
    this.clearing = true;
    ...
    this.currentFile = this.flatten(this.project.index).find(i => i.path === p);
    this.clearing = false;

    // Calculate word counts
    this.flatten(this.project.index).filter(i => typeof i.children === 'undefined').forEach(file => {
      file.words = this.wordCount(fs.readFileSync(path.resolve(path.dirname(this.projectPath), file.path), {
        encoding:'utf8',
        flag:'r'
      }));
  ...
  openItem(id)  {
    const file = this.flatten(this.project.index).find(i => this.idFromPath(i.path) === id);
        ...
        if (typeof item.children !== 'undefined') {
          ...
          >
            ...
            <span
              ...
              title="${typeof item.label === 'undefined' ? 'Click to add label.' : 'Labeled "' + this.getLabel(item.label)?.name + '". Click to edit.'}"
            >
              <div class="contextMenu">
                ${(this.project.labels.length) ?
                  this.project.labels.map(l =>
                    `<span
                      onclick="this.labelFile('${this.idFromPath(item.path)}', '${l.id}')"
                      data-label="${l.id}"
                    >${l.name}</span>`
                  ).join('') :
                  `<span class="--no-click">No labels.</span>`
                }
                ${typeof item.label === 'undefined' ? '' : `<span onclick="this.labelFile('${this.idFromPath(item.path)}', undefined)">Remove Label</span>`}
                <hr>
  ...
  resetEditor() {
    ...
    this.togglePreview = this.editor.toolbar.find(t => t.name === 'preview').action;
    if (this.readOnly && !this.editor.isPreviewActive()) setTimeout(() => {this.togglePreview(this.editor)}, 0);
  }
  ...
  renameItem(e) {
    ...
    const file = this.flatten(this.project.index).find(i => this.idFromPath(i.path) === e.parentNode.id);
  ...
  restoreOpenFolders() {
    ...
    for (const folder of toOpen) {
      try {
        // This may be wrong
        (<HTMLFormElement>document.getElementById(folder.id)).open = folder.open;
      } catch (err) {
  ...
  saveProject() {
    if (this.readOnly) return false;
    fs.writeFileSync(path.resolve(this.projectPath), JSON.stringify(this.project));
  }
  ...
  showModal(name) {
    ...
    switch (name) {
      case 'projectDetails':
        if (this.readOnly) return alert('You cannot update novel details while in Read Only mode.');
        modal = document.getElementById('projectDetails');

        // TODO: Show error if any modal cannot be found
        if (modal) {
          // Restore values
          (<HTMLTextAreaElement>document.getElementById('projectDetails__title')).value = this.project.metadata.title;
          (<HTMLTextAreaElement>document.getElementById('projectDetails__author')).value = this.project.metadata.author;
          (<HTMLTextAreaElement>document.getElementById('projectDetails__synopsis')).value = this.project.metadata.synopsis;
  ...
  startSprint(s = 0, m = 0, h = 0) {
    ...
    this.sprint = {
      ...
      interval: window.setInterval(() => {
        ...
        if (this.sprint) {
          let percent = this.sprint.total ? 1 - (timeLeft / this.sprint.total) : 0;
          ...
          if (timeleft) timeleft.innerText = `${hoursLeft}:${minutesLeft < 10 ? 0 : ''}${minutesLeft}:${secondsLeft < 10 ? 0 : ''}${secondsLeft}`;
        }
    ...
    if (wordsprint && wordsprint__popup) {
      ...
      wordsprint.innerHTML = '<span class="pie"><span class="segment"></span></span>'
  ...
  startRename(e)  {
    const isOpen = (e.tagName === 'SUMMARY' ? e.parentNode.open : this.currentFile);
    setTimeout(() => {
      if (isOpen !== (e.tagName === 'SUMMARY' ? e.parentNode.open : this.currentFile)) return;
      ...
      e.addEventListener('keydown', (event) => {
        ...
        if (event.key === 'Escape') {
          const file = this.flatten(this.project.index).find(i => this.idFromPath(i.path) === (e.id || e.parentNode.id));
          e.innerText = file.name;
          e.blur();
        }
      });
  ...
  updateDetails(toUpdate)  {
    ...
    for (var key of Object.keys(toUpdate)) {
      if (this.project.metadata[key] !== undefined) this.project.metadata[key] = toUpdate[key];
    }
    this.saveProject();
  }
  async updateGoals(updateHTML = true) {
    let goals = this.project.goals
      ...
      .map(g => {
        let newGoal = g;
        newGoal.done = Math.min(this.wordCountTotal() - g.startingWords, g.words);
    ...
    if (goals.length && goals[0].type === 'project' && goals[0].completed) {
      this.q('#projectGoalComplete__wordCount').innerText = goals[0].words.toLocaleString();
      this.q('#projectGoalComplete__days').innerText =
        Math.floor(
    ...
    if (goals.length) {
      this.q('#wordGoal').style = `--percent:${100*goals[0].done / goals[0].words}%`;
    ...
    var goals_element = goals
      .map(g => {
        return `<div ${g.completed ? 'class="completed"' : ''} style="--percent:${g.done * 100 / g.words}%">
          ${g.type} goal: ${g.done} / ${g.words} words
          <span title="Archive Goal" class="archive" onclick="this.archiveGoal('${g.id}')"><i class="far fa-trash-alt"></i></span>
        </div>`
      });
    ...
    if (updateHTML) {
      ...
      if (goals.length + acknowledged.length) this.q('#wordGoal__list').innerHTML = goals.join('') + acknowledged.join('');
      else this.q('#wordGoal__list').innerText = "You haven't set any goals yet.";
        ...
        .map(g => {
          ...
          if (g.history) {
            if (g.type === 'daily') {
              ...
              newGoals_element = `<div ${newGoal.daysCompleted === newGoal.history!.length ? 'class="completed"' : ''} style="--percent:${newGoal.daysCompleted * 100 / newGoal.history!.length}%">
                ${g.type} goal: completed ${newGoal.daysCompleted}/${newGoal.history!.length} days
              </div>`;
            } else {
              newGoals_element = `<div ${g.completed ? 'class="completed"' : ''} style="--percent:${g.done * 100 / g.words}%">
                ${g.type} goal: ${g.done} / ${g.words} words
              </div>`;
            }
          }
  ...
  async updateStats () {
    ...
    this.flatten(this.project.index).find((i: { path: any; }) => i.path === this.currentFile.path).words = stats.words;
    ...
    if (editor__stats && novelStats__open && novelStats__words) {
      ...
      if (this.project.history.wordCount) this.project.history.wordCount[(new Date()).toISOString().split('T')[0]] = this.wordCountTotal();

      novelStats__words.innerText = totalWords.toLocaleString() +
        ` (${(totalWords < this.startingWords ? '' : '+') + (totalWords - this.startingWords).toLocaleString()})`;
    }
  ...
  (async () => {
    ...
    if (releases__list) {
      try {
        ...
        releases__list.innerHTML = feed.items.map(item => {
